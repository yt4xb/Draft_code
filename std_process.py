...
def parse(start_time, end_time, line):
    ...
    time = datetime.strptime(split_line[1], "%Y%m%dT%H%M%S.%fZ")
...
def main(logfile, csvfile):
    ...
    start_time = datetime.strptime("20181114130000.000000", "%Y%m%dT%H%M%S.%f")
    end_time = datetime.strptime("20181114140000.000000", "%Y%m%dT%H%M%S.%f")
    w = open(csvfile, 'w+')
    r = open(filename, 'r')
